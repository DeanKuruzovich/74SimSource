...
test('74414 exists in CHIPS_BLOCK_25', () => {
  assert(CHIPS_BLOCK_25['74414'], 'chip missing');
});
test('74414 stub outputs all LOW', () => {
  const comp = makeComp('74414');
...
// ─────────────────────────────────────────────────────
test('74416 exists', () => {
  assert(CHIPS_BLOCK_25['74416'], 'chip missing');
});
test('74416 OEn=1 → all HiZ', () => {
  const comp = makeComp('74416');
...
// ─────────────────────────────────────────────────────
test('74417 exists', () => {
  assert(CHIPS_BLOCK_25['74417'], 'chip missing');
});
test('74417 CLRn=0 → Q=0', () => {
  const comp = makeComp('74417');
...
// ─────────────────────────────────────────────────────
test('74424 exists', () => {
  assert(CHIPS_BLOCK_25['74424'], 'chip missing');
});
test('74424 stub outputs all HiZ', () => {
  const comp = makeComp('74424');
...
// ─────────────────────────────────────────────────────
test('74425 exists', () => {
  assert(CHIPS_BLOCK_25['74425'], 'chip missing');
});
test('74425 En=0 → pass-through', () => {
  const comp = makeComp('74425');
...
// ─────────────────────────────────────────────────────
test('74426 exists', () => {
  assert(CHIPS_BLOCK_25['74426'], 'chip missing');
});
test('74426 En=1 → pass-through', () => {
  const comp = makeComp('74426');
...
// ─────────────────────────────────────────────────────
test('74433 exists', () => {
  assert(CHIPS_BLOCK_25['74433'], 'chip missing');
});
test('74433 starts empty (EF=1, FF=0)', () => {
  const comp = makeComp('74433');
...
// ─────────────────────────────────────────────────────
test('74436 exists', () => {
  assert(CHIPS_BLOCK_25['74436'], 'chip missing');
});
test('74436 OEn=0,EN=1 → Y=A', () => {
  const comp = makeComp('74436');
...
// ─────────────────────────────────────────────────────
test('74437 exists', () => {
  assert(CHIPS_BLOCK_25['74437'], 'chip missing');
});
test('74437 gate type is LINE_DRIVER_6X', () => {
  assertEqual(CHIPS_BLOCK_25['74437'].gates[0].type, 'LINE_DRIVER_6X', 'gate type');
});
test('74437 OEn=0,EN=1 → Y=A', () => {
  const comp = makeComp('74437');
...
// ─────────────────────────────────────────────────────
test('74440 exists', () => {
  assert(CHIPS_BLOCK_25['74440'], 'chip missing');
});
test('74440 OE1n=0,OE2n=0,DIR=0 → A→B', () => {
  const comp = makeComp('74440');
...
// ─────────────────────────────────────────────────────
test('74441 exists', () => {
  assert(CHIPS_BLOCK_25['74441'], 'chip missing');
});
test('74441 OE1n=0,OE2n=0,DIR=0 → B=!A', () => {
  const comp = makeComp('74441');
  setPins(comp, { OE1n:0,OE2n:0,DIR:0, A1:1,A2:0,A3:1,A4:0 });
  evalGate(comp);
...
// ─────────────────────────────────────────────────────
test('74442 exists', () => {
  assert(CHIPS_BLOCK_25['74442'], 'chip missing');
});
test('74442 OE1n=0,OE2n=0,DIR=0 → A→B', () => {
  const comp = makeComp('74442');
  setPins(comp, { OE1n:0,OE2n:0,DIR:0, A1:1,A2:0,A3:1,A4:0 });
  evalGate(comp);
  assertEqual(getPin(comp,'B1'), 1, 'B1=A1');
  assertEqual(getPin(comp,'B2'), 0, 'B2=A2');
...
// ─────────────────────────────────────────────────────
test('74443 exists', () => {
  assert(CHIPS_BLOCK_25['74443'], 'chip missing');
});
test('74443 OE1n=0,OE2n=0,DIR=0 → B=!A', () => {
  const comp = makeComp('74443');
  setPins(comp, { OE1n:0,OE2n:0,DIR:0, A1:1,A2:0,A3:1,A4:0 });
  evalGate(comp);
  assertEqual(getPin(comp,'B1'), 0, 'B1=!A1');
  assertEqual(getPin(comp,'B2'), 1, 'B2=!A2');
...
// ─────────────────────────────────────────────────────
test('74444 exists', () => {
  assert(CHIPS_BLOCK_25['74444'], 'chip missing');
});
test('74444 OE1n=0,OE2n=0,DIR=0 → A→B non-inv', () => {
  const comp = makeComp('74444');
  setPins(comp, { OE1n:0,OE2n:0,DIR:0, A1:1,A2:0,A3:1,A4:0 });
  evalGate(comp);
  assertEqual(getPin(comp,'B1'), 1, 'B1=A1');
  assertEqual(getPin(comp,'B2'), 0, 'B2=A2');
...
// ─────────────────────────────────────────────────────
test('74445 exists', () => {
  assert(CHIPS_BLOCK_25['74445'], 'chip missing');
});
test('74445 gate type is BCD_DECIMAL', () => {
  assertEqual(CHIPS_BLOCK_25['74445'].gates[0].type, 'BCD_DECIMAL', 'gate type');
});
test('74445 0000 → Y0 LOW (active LOW output)', () => {
  const comp = makeComp('74445');
