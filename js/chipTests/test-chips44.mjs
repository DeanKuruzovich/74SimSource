...
const EXPECTED_IDS = [
  '74x907','74x908','74x909','74x910','74x913','74x914','74x915','74x918',
  '74x920','74x921','74x922','74x923','74x925','74x926','74x927','74x928',
];

const EXPECTED_SPECS = {
  '74x907': { pins: 14, gnd:  7, vcc: 14 },
  '74x908': { pins:  8, gnd:  4, vcc:  8 },
  '74x909': { pins: 14, gnd:  7, vcc: 14 },
  '74x910': { pins: 18, gnd:  9, vcc: 18 },
  '74x913': { pins: 24, gnd: 12, vcc: 24 },
  '74x914': { pins: 14, gnd:  7, vcc: 14 },
  '74x915': { pins: 18, gnd:  9, vcc: 18 },
  '74x918': { pins: 14, gnd:  7, vcc: 14 },
  '74x920': { pins: 22, gnd: 12, vcc: 22 },
  '74x921': { pins: 18, gnd:  9, vcc: 18 },
  '74x922': { pins: 18, gnd:  9, vcc: 18 },
  '74x923': { pins: 20, gnd: 10, vcc: 20 },
  '74x925': { pins: 16, gnd:  8, vcc: 16 },
  '74x926': { pins: 16, gnd:  8, vcc: 16 },
  '74x927': { pins: 16, gnd:  8, vcc: 16 },
  '74x928': { pins: 16, gnd:  8, vcc: 16 },
};

const OC_IDS = ['74x907','74x909'];
...
const STUB_CONFIGS = [
  { id: '74x910', outputs: [] },   // bidir D* pins, no driven outputs
  { id: '74x913', outputs: ['DIS','a','b','c','d','e','f','g','COL'] },
  { id: '74x915', outputs: ['Q0','Q1','Q2','Q3','INV'] },
  { id: '74x920', outputs: ['DO0','DO1','DO2','DO3'] },
  { id: '74x921', outputs: [] },   // bidir D* pins, no driven outputs
  { id: '74x922', outputs: ['C1','C2','C3','C4','DA','A','B','C','D'] },
  { id: '74x923', outputs: ['C1','C2','C3','C4','DA','A','B','C','D','E'] },
  { id: '74x925', outputs: ['a','b','c','d','e','f','g','D1','D2','D3','D4'] },
  { id: '74x926', outputs: ['a','b','c','d','e','f','g','D1','D2','D3','D4'] },
  { id: '74x927', outputs: ['a','b','c','d','e','f','g','D1','D2','D3','D4'] },
  { id: '74x928', outputs: ['a','b','c','d','e','f','g','D1','D2','D3','D4'] },
];
...
{
  const { world, chip, wm } = setupChipWithPower('74x907');
...
{
  const { world, chip, wm } = setupChipWithPower('74x909');
...
// C1: 74908 - Dual NAND (8 pin relay driver), truth table for both gates
{
  ...
  for (const [inA, inB, out] of gates) {
    // Both HIGH → NAND = LOW
    {
      const { world, chip, wm } = setupChipWithPower('74x908');
...
// C2: 74914 - Hex Schmitt trigger inverter (NOT logic)
{
  ...
  for (const [inp, out] of gates) {
    {
      const { world, chip, wm } = setupChipWithPower('74x914');
...
// C3: 74918 - Dual NAND (14 pin, same logic as 74908)
{
  ...
  for (const [inA, inB, out] of gates) {
    // Both HIGH → NAND = LOW
    {
      const { world, chip, wm } = setupChipWithPower('74x918');
      connectHigh(wm, chip, inA); connectHigh(wm, chip, inB);
      const sim = new CircuitSimulator(); sim.evaluate(world, [chip], wm);
