// test-chips48.mjs - Tests for all chips defined in js/chips/chips48.js
// Chips under test:
//   74x1073   : 16 bit bus termination w/ bus-hold              (GENERIC_STUB, 20 pin)
//   74x1074   : Dual D neg edge FF async preset/clear           (GENERIC_STUB, 14 pin)
//   74x1181   : 4 bit ALU                                       (GENERIC_STUB, 24 pin)
//   74x1240   : Octal buf/driver inverting TRI STATE            (GENERIC_STUB, 20 pin)
//   74x1241   : Octal buf/driver non-inv TRI STATE              (GENERIC_STUB, 20 pin)
//   74x1242   : Quad bus transceiver inverting TRI STATE        (GENERIC_STUB, 14 pin)
//   74x1243   : Quad bus transceiver non-inv TRI STATE          (GENERIC_STUB, 14 pin)
//   74x1244   : Octal buf/driver non-inv TRI STATE (x244 lp)   (GENERIC_STUB, 20 pin)
//   74x1245   : Octal bus transceiver TRI STATE                 (GENERIC_STUB, 20 pin)
//   74x1280   : 9 bit parity gen/checker w/ reg outputs         (GENERIC_STUB, 20 pin)
//   74x1284   : Parallel printer interface transceiver          (GENERIC_STUB, 20 pin)
//   74x1394   : 2 bit GTLP transceiver                         (GENERIC_STUB, 16 pin)
//   74x1395   : Dual 1 bit GTLP transceiver                    (GENERIC_STUB, 20 pin)
//   74x1404   : Oscillator driver (Schmitt trigger)             (GENERIC_STUB, 8 pin)
//   74x1620   : Octal bus transceiver inverting TRI STATE       (GENERIC_STUB, 20 pin)
//   74x1621   : Octal bus transceiver non-inv OC                (GENERIC_STUB, 20 pin)
...
const EXPECTED_IDS = [
  '74x1073','74x1074','74x1181',
  '74x1240','74x1241','74x1242','74x1243','74x1244','74x1245',
  '74x1280','74x1284','74x1394','74x1395','74x1404','74x1620','74x1621',
];

const EXPECTED_SPECS = {
  '74x1073': { pins: 20, gnd: 10, vcc: 20 },
  '74x1074': { pins: 14, gnd:  7, vcc: 14 },
  '74x1181': { pins: 24, gnd: 12, vcc: 24 },
  '74x1240': { pins: 20, gnd: 10, vcc: 20 },
  '74x1241': { pins: 20, gnd: 10, vcc: 20 },
  '74x1242': { pins: 14, gnd:  7, vcc: 14 },
  '74x1243': { pins: 14, gnd:  7, vcc: 14 },
  '74x1244': { pins: 20, gnd: 10, vcc: 20 },
  '74x1245': { pins: 20, gnd: 10, vcc: 20 },
  '74x1280': { pins: 20, gnd: 10, vcc: 20 },
  '74x1284': { pins: 20, gnd: 10, vcc: 20 },
  '74x1394': { pins: 16, gnd:  8, vcc: 16 },
  '74x1395': { pins: 20, gnd: 10, vcc: 20 },
  '74x1404': { pins:  8, gnd:  4, vcc:  8 },
  '74x1620': { pins: 20, gnd: 10, vcc: 20 },
  '74x1621': { pins: 20, gnd: 10, vcc: 20 },
};

console.log('\n=== SECTION S: Structure ===');

assert(typeof CHIPS_BLOCK_48 === 'object', 'CHIPS_BLOCK_48 is exported object');
assert(Object.keys(CHIPS_BLOCK_48).length === 16, 'CHIPS_BLOCK_48 has 16 chips');

for (const id of EXPECTED_IDS) {
  const cd = CHIPS_BLOCK_48[id];
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION A - All stub chips: all output pins are HiZ
// ─────────────────────────────────────────────────────────────────────────────
