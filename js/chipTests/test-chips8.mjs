...
const EXPECTED_CHIP_IDS = [
  '7449','7450','7452','7453','7455','7456','7457',
  '7458','7459','7460','7461','7462','7463','7464','7465','7467',
];
...
console.log('\nG1: 7449 - BCD to 7-segment (OC)');
{
  ...
  const { world, chip, wm } = setupChipWithPower('7449');
  ...
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7449');
...
console.log('\nG2: 7450 - Dual 2-wide 2 input AOI');
{
  ...
  // All LOW → NOT(0|0)=1
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7450');
  ...
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7450');
  ...
  const { world: w3, chip: c3, wm: wm3 } = setupChipWithPower('7450');
...
{
  // All LOW → 0
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7452');
  ...
  // A1=A2=A3=1 → 1
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7452');
  ...
  // B1=B2=1 → 1
  const { world: w3, chip: c3, wm: wm3 } = setupChipWithPower('7452');
...
{
  // All LOW → NOT(0)=1
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7453');
  applyInputs(wm1, c1, { A1:0, A2:0, A3:0, B1:0, B2:0, C1:0, C2:0, D1:0, D2:0 });
  const sim1 = simulate(w1, c1, wm1);
  assert(isHigh(getPinVoltage(sim1, findPin(c1, 'Y'))), '7453 AOI_3222(all 0)=1');
  // A1=A2=A3=1 → NOT(1)=0
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7453');
  applyInputs(wm2, c2, { A1:1, A2:1, A3:1, B1:0, B2:0, C1:0, C2:0, D1:0, D2:0 });
  const sim2 = simulate(w2, c2, wm2);
  assert(isLow(getPinVoltage(sim2, findPin(c2, 'Y'))), '7453 AOI_3222(A=1,1,1)=0');
  // D1=D2=1 → NOT(1)=0
  const { world: w3, chip: c3, wm: wm3 } = setupChipWithPower('7453');
...
{
  // All LOW → NOT(0)=1
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7455');
  ...
  // A all HIGH → NOT(1)=0
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7455');
  ...
  // B all HIGH → NOT(1)=0
  const { world: w3, chip: c3, wm: wm3 } = setupChipWithPower('7455');
  ...
  const { world: w4, chip: c4, wm: wm4 } = setupChipWithPower('7455');
...
console.log('\nG9: 7460 - Dual 4 input AND expander');
{
  ...
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7460');
  applyInputs(wm1, c1, { '1A':1, '1B':1, '1C':1, '1D':1 });
  const sim1 = simulate(w1, c1, wm1);
  assert(isHigh(getPinVoltage(sim1, findPin(c1, '1X'))), '7460 AND(1,1,1,1)=1');
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7460');
...
{
  // All LOW → 0
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7462');
  ...
  // A1=A2=A3=1 → 1
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7462');
...
console.log('\nG12: 7463 - Hex current sensing inverters');
{
  for (const n of ['1','2','3','4','5','6']) {
    const { world, chip, wm } = setupChipWithPower('7463');
    ...
    const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7463');
...
{
  // All LOW → NOT(0)=1
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7464');
  ...
  // A all HIGH → NOT(1)=0
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7464');
  ...
  // B all HIGH → NOT(1)=0
  const { world: w3, chip: c3, wm: wm3 } = setupChipWithPower('7464');
  ...
  // C=1,1 → NOT(1)=0
  const { world: w4, chip: c4, wm: wm4 } = setupChipWithPower('7464');
  ...
  // D=1,1 → NOT(1)=0
  const { world: w5, chip: c5, wm: wm5 } = setupChipWithPower('7464');
...
{
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7465');
  applyInputs(wm1, c1, { A1:0, A2:0, A3:0, A4:0, B1:0, B2:0, B3:0, C1:0, C2:0, D1:0, D2:0 });
  const sim1 = simulate(w1, c1, wm1);
  const v1 = getPinVoltage(sim1, findPin(c1, 'Y'));
  assert(isHigh(v1) || v1 === undefined, '7465 AOI_4322(all 0)=1/HiZ');
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7465');
  applyInputs(wm2, c2, { A1:1, A2:1, A3:1, A4:1, B1:0, B2:0, B3:0, C1:0, C2:0, D1:0, D2:0 });
  const sim2 = simulate(w2, c2, wm2);
...
console.log('\nG15: 7467 - JK controller-device FF (AND-gated)');
{
  ...
  const { world: w1, chip: c1, wm: wm1 } = setupChipWithPower('7467');
  ...
  const { world: w2, chip: c2, wm: wm2 } = setupChipWithPower('7467');
