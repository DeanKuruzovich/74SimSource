...
const EXPECTED_CHIP_IDS = [
  '74206', '74207', '74208', '74209', '74210', '74211', '74212', '74213',
  '74214', '74215', '74216', '74217', '74218', '74219', '74222', '74224',
];
...
{
  const { world, chip } = setupWorld('74206');
...
{
  const { world, chip } = setupWorld('74207');
...
{
  const { world, chip } = setupWorld('74208');
...
console.log('\nG4: 74209 - 1024x1 RAM');
{
  const { world, chip } = setupWorld('74209');
...
{
  const { world, chip } = setupWorld('74210');
...
{
  const { world, chip } = setupWorld('74211');
...
{
  const { world, chip } = setupWorld('74212');
...
console.log('\nG8: 74213 - 16x12 RAM');
{
  const { world, chip } = setupWorld('74213');
  const wmW = makeWm(chip, { A0:0,A1:0,A2:0,A3:0,
...
console.log('\nG9: 74214 - 1024x1 RAM');
{
  const { world, chip } = setupWorld('74214');
...
{
  const { world, chip } = setupWorld('74215');
  const wm = makeWm(chip, { A0:0,A1:0,A2:0,A3:0,A4:0,A5:0,A6:0,A7:0,A8:0,A9:0,
    DIN:0, PD:1, WE:1 });
  const sim = simulate(world, chip, wm);
  const v = getPinVoltage(sim, findPin(chip, 'DOUT'));
...
{
  const { world, chip } = setupWorld('74216');
...
{
  const { world, chip } = setupWorld('74217');
...
{
  const { world, chip } = setupWorld('74218');
...
{
  const { world, chip } = setupWorld('74219');
...
{
  const { world, chip } = setupWorld('74222');
...
{
  const { world, chip } = setupWorld('74224');
  const wm = makeWm(chip, { DIN0:0,DIN1:0,DIN2:0,DIN3:0,
