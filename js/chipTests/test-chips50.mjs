// test-chips50.mjs - Tests for all chips defined in js/chips/chips50.js
// Chips under test (all GENERIC_STUB):
//   74x1808     : Hex 2 input AND driver               (GENERIC_STUB, 20-pin)
//   74x1811     : FM/MFM/DM encoder/decoder 20 MHz     (GENERIC_STUB, 24-pin)
//   74x1821     : 10 bit bus interface FF TRI           (GENERIC_STUB, 24-pin)
//   74x1823     : 9 bit bus interface FF with clear TRI (GENERIC_STUB, 24-pin)
//   74x1832     : Hex 2 input OR driver                 (GENERIC_STUB, 20-pin)
//   74x1841     : 10 bit bus interface latch TRI        (GENERIC_STUB, 24-pin)
//   74x1843     : 9 bit bus interface latch w/clear TRI (GENERIC_STUB, 24-pin)
//   74x2003     : 8 bit level translator                (GENERIC_STUB, 20-pin, bidir)
//   74x2010     : 10 bit level translator               (GENERIC_STUB, 24-pin, bidir)
//   74x2014     : 4 bit GTL to TTL transceiver TRI+OC   (GENERIC_STUB, 14-pin, bidir)
//   74x2125     : Quad bus buffer TRI 25Ω               (GENERIC_STUB, 14-pin)
//   74x2150     : 512×8 cache address comparator        (GENERIC_STUB, 24-pin)
//   74FCT2151   : 8:1 mux 25Ω                          (GENERIC_STUB, 16-pin)
//   74FCT2153   : Dual 4:1 mux 25Ω                     (GENERIC_STUB, 16-pin)
//   74FCT2157   : Quad 2:1 mux 25Ω                     (GENERIC_STUB, 16-pin)
//   74x2161     : Sync presettable 4 bit counter 25Ω   (GENERIC_STUB, 16-pin)
...
const EXPECTED_IDS = [
  ...
  '74x1832',
  '74x1841','74x1843',
  '74x2003','74x2010','74x2014',
  '74x2125','74x2150',
  '74FCT2151','74FCT2153','74FCT2157',
  '74x2161',
];

const EXPECTED_SPECS = {
  '74x1808':   { pins: 20, gnd: 10, vcc: 20 },
  '74x1811':   { pins: 24, gnd: 12, vcc: 24 },
  '74x1821':   { pins: 24, gnd: 12, vcc: 24 },
  '74x1823':   { pins: 24, gnd: 12, vcc: 24 },
  '74x1832':   { pins: 20, gnd: 10, vcc: 20 },
  '74x1841':   { pins: 24, gnd: 12, vcc: 24 },
  '74x1843':   { pins: 24, gnd: 12, vcc: 24 },
  '74x2003':   { pins: 20, gnd: 10, vcc: 20 },
  '74x2010':   { pins: 24, gnd: 12, vcc: 24 },
  '74x2014':   { pins: 14, gnd:  7, vcc: 14 },
  '74x2125':   { pins: 14, gnd:  7, vcc: 14 },
  '74x2150':   { pins: 24, gnd: 12, vcc: 24 },
  '74FCT2151': { pins: 16, gnd:  8, vcc: 16 },
  '74FCT2153': { pins: 16, gnd:  8, vcc: 16 },
  '74FCT2157': { pins: 16, gnd:  8, vcc: 16 },
  '74x2161':   { pins: 16, gnd:  8, vcc: 16 },
};

console.log('\n=== SECTION S: Structure ===');

assert(typeof CHIPS_BLOCK_50 === 'object', 'CHIPS_BLOCK_50 is exported object');
assert(Object.keys(CHIPS_BLOCK_50).length === 16, 'CHIPS_BLOCK_50 has 16 chips');

for (const id of EXPECTED_IDS) {
  const cd = CHIPS_BLOCK_50[id];
