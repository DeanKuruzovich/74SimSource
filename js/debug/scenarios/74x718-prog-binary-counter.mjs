// ── 74x718 Programmable Modulo-N (mod-16 / binary) counter ──────────────────
// The 74x718 (js/chips/chips37.js) is the Motorola MC4018 "Programmable
// Modulo-N Counter", hexadecimal (binary) member. It drives the
// COUNTER_PROG_MODN_4018 primitive (js/specificChipsSim.js) with gate.mod = 16 —
// the same primitive that runs the mod-10 74x716 (MC4016) and the 74x418, just a
// different modulus.
//
// Behaviour verified against the Motorola "SN54LS/74LS716 • SN54LS/74LS718 —
// Programmable Modulo-N Counters" datasheet (1986 Motorola FAST and LS TTL Data,
// pp. 5-352..5-353; see chips37.js header for the full citation). This scenario
// exercises:
//   • count up 0..15 in binary on the Clock rising edge,
//   • wrap 15 -> 0,
...
//     (internal pull-up).
//
// Run:  node js/debug/scenarios/74x718-prog-binary-counter.mjs  (exits non-zero on fail)
...
const chip = new ChipComponent('74x718');
chip.place(0, 0, 2, 4);
...
function solve() {
  ...
  const wirePin = (name, hi) => {
    ...
    if (!p) throw new Error(`74x718 has no pin named ${name}`);
...
// ── 2. Count up 0..15 in binary, checking each step and the Bus carry ─────────
for (let n = 1; n <= 15; n++) {
  clock();
...
setData(11);
st.pe = 0; solve();
assert(qval() === 11, `PE LOW should load D0-D3 = 11 with no clock, got ${qval()}`);
st.pe = 1; solve();
assert(qval() === 11, `count should hold 11 after PE release, got ${qval()}`);
clock();
assert(qval() === 12, `should count on from the loaded value (11 -> 12), got ${qval()}`);
...
clock();
assert(qval() === 14, `Gate HIGH + a clock edge should load 14, got ${qval()}`);
st.gate = 0; solve();
clock();
assert(qval() === 15, `with Gate LOW it should count again (14 -> 15), got ${qval()}`);

// ── 7. R gates the Bus carry at terminal count 15 ────────────────────────────
setData(15);
...
console.log(`74x718-prog-binary-counter: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
