// ── 74x4060 14-stage binary ripple counter + oscillator regression ───────────
// The 74x4060 (js/chips/chips58.js) drives the COUNTER_BIN_OSC_14_CLKO engine
// primitive. The counter advances on the HIGH-to-LOW (falling) edge of CLKI;
// a HIGH on CLR clears every stage and parks the oscillator buffers.
//
// Stages 1-3 and stage 11 are internal, so the accessible taps are Q4..Q10 and
// Q12..Q14. Q4 = bit 3 of the count (toggles every 16 clocks), Q14 = bit 13.
//
...
//
// Run:  node js/debug/scenarios/74x4060-binary-counter-osc.mjs  (non-zero on fail)
...
const chip = new ChipComponent('74x4060');
chip.place(0, 0, 2, 4);
...
function apply({ clk, clr = 0 }) {
  ...
  const wirePin = (name, bit) => {
    ...
    if (!p) throw new Error(`74x4060 has no pin named ${name}`);
...
for (const q of ['Q4','Q5','Q6','Q7','Q8','Q9','Q10','Q12','Q13','Q14'])
  assert(isLow(read(q)), `CLR HIGH: ${q} must be LOW`);
...
apply({ clk: 0, clr: 0 });
assert(isLow(read('Q4')), 'after CLR release, Q4 still LOW (count 0)');

// Q4 is bit 3 of the count, so it first goes HIGH at count 8 (its full cycle is
// 16 counts → divide-by-16 in frequency). Q5 is bit 4 (HIGH at count 16), etc.
const QBIT = { Q4: 3, Q5: 4, Q6: 5, Q7: 6, Q8: 7, Q9: 8, Q10: 9 };
...
// ── 2. The accessible taps follow the binary count ────────────────────────────
for (let i = 0; i < 8;  i++) tick();           // count 8
expectCount(8,  'Q4 first HIGH');
for (let i = 0; i < 8;  i++) tick();           // count 16
expectCount(16, 'Q5 HIGH, Q4 LOW');
for (let i = 0; i < 8;  i++) tick();           // count 24
expectCount(24, 'Q4 and Q5 HIGH');
for (let i = 0; i < 8;  i++) tick();           // count 32
expectCount(32, 'Q6 HIGH');

// ── 3. Falling edge advances, rising edge does not ────────────────────────────
...
// Walk 8 falling edges from here; Q4 (bit 3) must set exactly on the 8th.
...
apply({ clk: 1, clr: 0 });
assert(isLow(read('CLKO')),  'CLKI HIGH, running: CLKO is the complement (LOW)');
...
console.log(`74x4060-binary-counter-osc: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
