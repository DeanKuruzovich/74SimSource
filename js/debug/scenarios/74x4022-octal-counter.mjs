// ── 74x4022 octal (÷8) Johnson counter, decoded — regression ────────────────
// The 74x4022 (js/chips/chips57.js) is the upgraded former stub: a 16-pin
// divide-by-8 Johnson counter, functionally identical to the CD4022B. It shares
// the COUNTER_OCTAL_DECODED primitive with the CD4022 entry. This guards its DB
// entry after the stub fix that corrected a wrong 14-pin map to the verified
// 16-pin one (Q0=2, Q1=1, Q2=3, Q3=7, Q4=11, Q5=4, Q6=5, Q7=10, CO=12, CI=13,
// CLK=14, MR=15, VDD=16, GND=8): the RISING-edge clock gated by active-HIGH
// CLOCK INHIBIT (CI), the active-HIGH asynchronous RESET (MR), mod-8 wrap, and
// CARRY OUT HIGH for counts 0-3 / LOW for counts 4-7.
//
// Method: place ONE 74x4022 and keep the same chip + sim instance across the run
...
//
// Run:  node js/debug/scenarios/74x4022-octal-counter.mjs  (exits non-zero on fail)
...
const chip = new ChipComponent('74x4022');
chip.place(0, 0, 2, 4);
...
function apply({ clk, reset = false, inhibit = false }) {
  ...
  const wirePin = (name, row) => {
    ...
    if (!p) throw new Error(`74x4022 has no pin named ${name}`);
...
console.log(`74x4022-octal-counter: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
