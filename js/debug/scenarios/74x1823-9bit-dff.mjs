// ── 74x1823 9-bit bus-interface D flip-flop regression ───────────────────────
// The 74x1823 (js/chips/chips50.js) is the single-width member of the '16823
// Widebus family: a 9-bit D-type register with an asynchronous active-LOW clear
// (CLRn), an active-LOW clock enable (CLKENn) and active-LOW 3-state output
// control (OEn). It rides the D_FF_9BIT_CLR_CE_TRI engine primitive — the same
// function as the 74x823. This scenario guards the DB entry: the corrected pin
// map (OEn=1, D0-D8=2-10, CLRn=11, CLK=13, CLKENn=14, Q8-Q0=15-23), the
// non-inverting rising-edge capture (Q=D), the active-LOW clock-enable hold, the
// asynchronous clear, and 3-state output control.
//
// D_FF_9BIT_CLR_CE_TRI contract (js/specificChipsSim.js):
//   inputs:  [OEn, CLRn, CLKENn, CLK, D0..D8]   (OEn/CLRn/CLKENn all active LOW)
//   outputs: [Q0..Q8]                           rising-edge capture when CLKENn LOW
//
// Function table (each 9-bit flip-flop, TI SN74ABTH16823 SCBS664B, verified):
//   OEn CLRn CLKENn CLK  D | Q
//    L   L     X     X   X | L     (async clear dominates)
//    L   H     L     ↑   H | H     (CLKENn low: load D on rising edge)
//    L   H     H     X   X | hold  (CLKENn high: clock disabled)
//    H   X     X     X   X | Z     (output disabled, FF state kept)
//
// Run:  node js/debug/scenarios/74x1823-9bit-dff.mjs   (exits non-zero on fail)
...
const chip = new ChipComponent('74x1823');
chip.place(0, 0, 2, 4);
...
function apply(st) {
  ...
  const wirePin = (name, row) => {
    ...
    if (!p) throw new Error(`74x1823 has no pin named ${name}`);
...
// not clearing (CLRn high), clock enabled (CLKENn low), clock low.
...
// ── 2. Active-LOW clock enable: CLKENn HIGH holds through a rising edge ───────
load(0b000111000);
assert(qbits() === 0b000111000, `preload: got ${bin9(qbits())}`);
st.cen = 1;                     // disable clock
setD(0b111111111);
st.clk = 0; solve();
st.clk = 1; solve();            // rising edge, but CLKENn high → no capture
assert(qbits() === 0b000111000, `CLKENn high should hold, got ${bin9(qbits())}`);
st.cen = 0;                     // re-enable; now a real edge loads the new data
st.clk = 0; solve();
st.clk = 1; solve();
assert(qbits() === 0b111111111, `CLKENn low reload: got ${bin9(qbits())}`);
...
if (failures.length) {
  console.error(`74x1823: FAIL (${failures.length})`);
  for (const f of failures) console.error('  - ' + f);
  process.exit(1);
}
console.log('74x1823: PASS — clear, non-inverting load, CLKENn hold, 3-state output all correct');
