// ── 74x716 Programmable Modulo-N (mod-10 / decade) counter ──────────────────
// The 74x716 (js/chips/chips37.js) is the Motorola MC4016 "Programmable
// Modulo-N Counter", decade member. It drives the COUNTER_PROG_MODN_4018
// primitive (js/specificChipsSim.js) with gate.mod = 10 — the same primitive
// that runs the mod-16 74x418 (MC4018), just a different modulus.
//
// Behaviour verified against the Motorola "Programmable Modulo-N Counters —
// MC4316 thru MC4319 / MC4016 thru MC4019" 3-page data sheet (see chips37.js
// header for the full citation). This scenario exercises:
//   • count up 0..9 in BCD on the Clock rising edge,
//   • wrap 9 -> 0,
//   • Bus (carry) HIGH only at the terminal count 9,
...
//     (internal pull-up).
//
// Run:  node js/debug/scenarios/74x716-prog-decade-counter.mjs  (exits non-zero on fail)
...
const chip = new ChipComponent('74x716');
chip.place(0, 0, 2, 4);
...
function solve() {
  ...
  const wirePin = (name, hi) => {
    ...
    if (!p) throw new Error(`74x716 has no pin named ${name}`);
...
// ── 2. Count up 0..9 in BCD, checking each step and the Bus carry ────────────
for (let n = 1; n <= 9; n++) {
  clock();
  assert(qval() === n, `after ${n} clocks count should be ${n}, got ${qval()}`);
  assert(isHigh(read('Bus')) === (n === 9),
    `Bus should be ${n === 9 ? 'HIGH' : 'LOW'} at count ${n}`);
}

// ── 3. Wrap 9 -> 0 and Bus drops ─────────────────────────────────────────────
clock();
assert(qval() === 0, `mod-10 should wrap 9 -> 0, got ${qval()}`);
assert(!isHigh(read('Bus')), 'Bus should drop LOW after wrapping to 0');

// ── 4. PE: asynchronous parallel load (no clock edge needed) ─────────────────
setData(6);
st.pe = 0; solve();
assert(qval() === 6, `PE LOW should load D0-D3 = 6 with no clock, got ${qval()}`);
st.pe = 1; solve();
assert(qval() === 6, `count should hold 6 after PE release, got ${qval()}`);
clock();
assert(qval() === 7, `should count on from the loaded value (6 -> 7), got ${qval()}`);
...
clock();
assert(qval() === 8, `Gate HIGH + a clock edge should load 8, got ${qval()}`);
st.gate = 0; solve();
clock();
assert(qval() === 9, `with Gate LOW it should count again (8 -> 9), got ${qval()}`);

// ── 7. R gates the Bus carry at terminal count 9 ─────────────────────────────
setData(9);
st.pe = 0; solve();            // jam to terminal count 9
st.pe = 1; solve();
assert(qval() === 9, `setup: should be at 9, got ${qval()}`);
assert(isHigh(read('Bus')), 'Bus should be HIGH at 9 with R HIGH');
st.r = 0; solve();
assert(!isHigh(read('Bus')), 'R LOW must force Bus LOW even at count 9');
st.r = null; solve();          // leave R floating
assert(isHigh(read('Bus')), 'floating R should enable Bus (internal pull-up) at 9');
st.r = 1; solve();

console.log(`74x716-prog-decade-counter: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
