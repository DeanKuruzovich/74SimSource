// ── CD40181 CMOS 4-bit ALU / function generator — regression ─────────────────
// The CD40181 (Batch 13, js/chips/chips147.js) is the CMOS twin of the 74181.
// It is purely combinational and reuses the existing ALU_4BIT engine primitive
// (js/specificChipsSim.js _evaluateAlu4Bit), which models the datasheet's
// ACTIVE-HIGH data convention (RCA CD40181B Fig. 2 / TI SN74181 Table 2).
//
// This guards: the verified CD40181B pin map (B0=1,A0=2,S3=3,S2=4,S1=5,S0=6,
// Cn=7,M=8,F0=9,F1=10,F2=11,VSS=12,F3=13,A=B=14,P=15,Cn4=16,G=17,B3=18,A3=19,
// B2=20,A2=21,B1=22,A1=23,VDD=24), and a representative slice of the function
// table:
...
//
// Method: place ONE CD40181 (no sequential state), drive every input pin from a
// rail, re-solve, read F0–F3 / Cn4 / AeqB off the pins.
//
// Run:  node js/debug/scenarios/cd40181-alu.mjs   (exits non-zero on fail)
...
const chip = new ChipComponent('CD40181');
chip.place(0, 0, 2, 4);
...
function apply(a, b, s, m, cn) {
  ...
  const wirePin = (name, hi) => {
    ...
    if (!p) throw new Error(`CD40181 has no pin named ${name}`);
...
// ── Report ───────────────────────────────────────────────────────────────────
if (failures.length) {
  console.error(`CD40181 ALU regression FAILED (${failures.length}):`);
  for (const f of failures) console.error('  ✗', f);
  process.exit(1);
}
console.log('CD40181 ALU regression PASSED');
