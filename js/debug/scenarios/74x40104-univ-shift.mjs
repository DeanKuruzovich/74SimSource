// ── 74x40104 4-bit bidirectional UNIVERSAL shift register, tri-state — regression
// The 74x40104 (js/chips/chips65.js) was a GENERIC_STUB upgraded in place to the
// SHIFT_REG_4BIT_UNIV_TRI primitive. It is the same functional device as the CMOS
// CD40104 (chips134.js) under the 74-series naming. Its hand-entered stub pinout
// was scrambled (issues.md C2) and has been corrected to the SGS-Thomson
// HCC/HCF40104B datasheet (June 1989). This guards the four S1/S0 modes plus the
// active-HIGH three-state output:
//   S1 S0 = 00 → synchronous CLEAR (reg → 0; this part has no reset pin)
...
// All actions occur on the rising CLK edge.
//
// Method mirrors cd40104-univ-shift.mjs: one chip + sim instance kept for the whole
// run so the register's sequential state persists; a clock pulse is LOW→HIGH→LOW.
//
// Run:  node js/debug/scenarios/74x40104-univ-shift.mjs   (exits non-zero on fail)
...
const chip = new ChipComponent('74x40104');
chip.place(0, 0, 2, 4);
...
function apply({ clk = 0, s0 = 0, s1 = 0, sr = 0, sl = 0, oe = 1, d = null } = {}) {
  ...
  const wirePin = (name, bit) => {
    ...
    if (!pin) throw new Error(`74x40104 has no pin named ${name}`);
...
console.log(`74x40104-univ-shift: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
