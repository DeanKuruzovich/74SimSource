// ── CD40193 presettable BINARY up/down counter, DUAL clock — regression ──────
// The CD40193 (js/chips/chips128.js) is the behavioral coverage of the
// COUNTER_UPDOWN_DC primitive (the dual-clock 74x193 binary up/down model). It
// guards the chip's DB entry against the coverage-plan's mis-hint: the plan maps
// CD40193 to COUNTER_BIN_UPDOWN_CD (the SINGLE-clock CD4516 model with a UP/DOWN
// pin + carry-in), but the real CD40193B is the 74193 family — two separate
...
// J1=LSB…J4=MSB), the BINARY mod-16 dual-clock count, the active-LOW asynchronous
// PRESET ENABLE jam-load, the active-HIGH asynchronous RESET (which dominates),
// and the active-LOW CARRY (count 15 with CLOCK UP low) / BORROW (count 0 with
...
// edge of CLOCK DOWN (CLOCK UP held HIGH); BINARY modulus 16. RESET HIGH clears to
// 0 asynchronously; PRESET ENABLE LOW jam-loads J1–J4 asynchronously. CARRY pulses
// LOW at 15 while CLOCK UP is LOW; BORROW pulses LOW at 0 while CLOCK DOWN is LOW.
//
// Method: place ONE CD40193 and keep the same chip + sim instance across the run
...
//
// Run:  node js/debug/scenarios/cd40193-updown-counter.mjs  (exits non-zero on fail)
...
const chip = new ChipComponent('CD40193');
chip.place(0, 0, 2, 4);
...
function apply(s) {
  ...
  const wirePin = (name, row) => {
    ...
    if (!p) throw new Error(`CD40193 has no pin named ${name}`);
...
// ── 1. Count up 0→15 in binary (rising-edge CLOCK UP) ────────────────────────
for (let n = 1; n <= 15; n++) {
  pulseUp(1);
  assert(count() === n, `up: after ${n} pulses count should be ${n}, got ${count()}`);
}

// ── 2. Weighting at 15 (Q4 Q3 Q2 Q1 = 1111) — full binary terminal count ─────
...
// ── 3. CARRY asserts LOW at 15 while CLOCK UP is LOW ─────────────────────────
st.up = 0; solve();                 // CLOCK UP low, still at 15
assert(count() === 15, `carry-check: count should still be 15, got ${count()}`);
assert(carry() === 0, `carry: should assert LOW at 15 with CLOCK UP low, got ${carry()}`);
st.up = 1; solve();                 // rising edge → wrap 15→0
...
// ── 5. Asynchronous PRESET ENABLE jam-load (J4..J1 = 1011 = decimal 13) ──────
st.j1 = 1; st.j2 = 0; st.j3 = 1; st.j4 = 1;  // value 0b1101 = 13
st.pe = 0;                                    // PRESET ENABLE LOW = active → async load
solve();                                      // PE low → async load J1..J4
assert(count() === 13, `preset: jam-load should give 13, got ${count()}`);
st.pe = 1; solve();                           // PE HIGH → counting (value persists)
assert(count() === 13, `preset: value should persist after PE high, got ${count()}`);
...
st.down = 1; solve();                // rising edge → wrap 0→15 (binary down-wrap)
...
st.j1 = 1; st.j2 = 1; st.j3 = 1; st.j4 = 1;  // would load 15
...
console.log(`cd40193-updown-counter: ${failures.length === 0 ? 'ALL PASS' : 'FAILURES'}`);
