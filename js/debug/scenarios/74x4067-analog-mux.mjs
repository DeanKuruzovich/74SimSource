// ── 74x4067 regression: 16-channel analog mux/demux (ANALOG_MUX_16) ─────────
// De-stubbed in chips58.js. Verifies the bidirectional transmission-gate
// behavior of the 24-pin part:
...
//
// Pinout verified against TI SCHS209D (CD74HC4067) Figure 4-1 + Table 4-1:
//   COMMON I/O(1), Y7..Y0 on 2-9, A=S0(10), B=S1(11), GND(12), D=S3(13),
//   C=S2(14), INH=E(15), Y15..Y8 on 16-23, VCC(24). No VEE pin. Because that
//   physical map is identical to the CD4067 entry, the hole mapping below
//   mirrors cd4067-analog-mux.mjs.
//
// Method: drive one channel hard to 5 V, hang a large (100 kΩ) pull-down on Z,
// and read Z. When that channel is selected the ~70 Ω ON path dominates and
// Z ≈ 5 V; otherwise Z is isolated and the pull-down holds it near 0 V.
//
// Run:  node js/debug/scenarios/74x4067-analog-mux.mjs  (exits non-zero on fail)
//
...
//            Y1=c12 · Y0=c13 · A=c14 · B=c15 · GND=c16
//   top:     VCC=c5 · Y8=c6 · Y9=c7 · Y10=c8 · Y11=c9 · Y12=c10 · Y13=c11 ·
...
// Build a 74x4067 with VCC/GND powered, INH = inhibit level, address bits
// {a,b,c,d}, channel Y5 (sel=5) hard-driven to 5 V, and a 100k pull-down on Z.
function build({ a, b, c, d, inh }) {
  const railHi = col => `0:0:power:${col}:1`;
  const railLo = col => `0:0:power:${col}:2`;
  return CircuitHarness.fromJSON({
    components: [
      { id: 10, type: 'chip', tileX: 0, tileY: 0, col: 5, row: 4, chipId: '74x4067' },
    ...
    wires: [
      { id: 1, startHoleId: railHi(5),  endHoleId: '0:0:main:5:3'  }, // VCC pin24 (top c5) → 5V
      { id: 2, startHoleId: railLo(16), endHoleId: '0:0:main:16:6' }, // GND pin12 (bot c16) → GND
...
{
  console.log('\n1. 74x4067 address D,C,B,A = 0,1,0,1 → channel 5 selected, Z ≈ 5V');
...
// with its own pull-down; Z is driven to 5V; DCBA = 1010 selects channel 10.
{
  ...
  const h = CircuitHarness.fromJSON({
    components: [
      { id: 20, type: 'chip', tileX: 0, tileY: 0, col: 5, row: 4, chipId: '74x4067' },
    ...
    wires: [
      { id: 1, startHoleId: '0:0:power:5:1',  endHoleId: '0:0:main:5:3'  }, // VCC (top c5)
      { id: 2, startHoleId: '0:0:power:16:2', endHoleId: '0:0:main:16:6' }, // GND (bot c16)
